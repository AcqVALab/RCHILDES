...
This workshop shows how you can use and analyze the *Child Language Data Exchange System* (CHILDES) data base using R. The RMarkdown document for the tutorial can be downloaded [here](https://slcladal.github.io/childes.Rmd) and the bib library [here](https://slcladal.github.io/bibliography.bib). 
...
```{r childes_01_40, message=F, warning=F}
write.table(childes, here::here("tables", "childes.txt"), sep = "\t", row.names = F)
```

# Case studies

Now that we have the data in a format that we can use, we can extract information from the table
